...
@section('user-content')
                            ...
                            @foreach( $DB_Destination->where('idDestination', $value->id_destination) as $values)
                                <div class="flex items-center text-sm">
                                    <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                        <img class="object-cover w-full h-full rounded-full" src="{{ asset('img/'.$values->Image) }}" alt="" loading="lazy" />
                                        <div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
                                    </div>
                                    <div>
                                        <a href="/destination/single-page/{{ $value->id_destination }}"><p class="font-semibold">{{ $values->Name_Destination }}</p></a>
                                        <p class="text-xs text-gray-600 dark:text-gray-400">{{ $values->Category }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </td>
                        ...
                        <td>
                            @foreach( $DB_Destination->where('idDestination', $value->id_destination) as $values)
                                <button type="button" 
                                        ...
                                        data-id="{{ $value->id_destination }}"
                                        data-price="{{ $values->Price_perticket }}" 
                        ...
                        <div class="p-4 md:p-5 flex flex-col items-center">
                            <div class="grid gap-4 mb-4 w-full">
                                <div>
                                    ...
                                    <input type="number" name="jumlah-ticket" id="qty" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Masukkan jumlah ticket" required="">
                                </div>
                                <div>
                                    <label for="tanggal" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tanggal</label>
                                    <input type="date" name="tanggal" id="date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required="">
                                </div>
                            </div>

                        
                            <!-- Submit Button -->
                            <button id="pay-button" type="submit" class="text-white inline-flex items-center bg-info hover:bg-info-dark focus:ring-4 focus:outline-none focus:ring-info font-medium rounded-lg text-sm px-5 py-2.5 text-center">
    ...
    $(document).ready(function () {
        ...
        $('[data-modal-toggle="crud-modal"]').on('click', function () {
            ...
            $('#crud-modal').find('.modal-destination-id').text(`Destination ID: ${destinationId}`);
            $('#crud-modal').find('.modal-price').text(`Price: Rp. ${price}`);
            $('#qty').val(); 
            $('#date').val(''); 
        });
